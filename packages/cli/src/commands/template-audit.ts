...
export function analyzeTemplateSource(template: TemplateMetadata, source: string): TemplateAuditItem {
  ...
  const defaultSec = duration
    ? (duration as typeof duration & { default_sec?: number }).default_sec ?? duration.max_sec
    : undefined;
  if (duration && defaultSec !== undefined && (defaultSec < duration.min_sec || defaultSec > duration.max_sec)) {
