...
> **在你的电脑上，把 HTML 变成视频。** 接上你本地的 coding agent（Open Design · Windsurf CLI · Trae CLI · Claude Code · Cursor · Codex · Gemini · Grok · Qwen · OpenCode · Copilot · Aider · Hermes · 或 Anthropic API），描述一个视频，**直接粘一个文章链接 / GitHub 仓库**，或上传一段人像口播视频，agent 就把它变成一支多帧、带动画的视频 —— 然后就在你这台机器上渲染成真实 MP4。一个 agent 循环、可插拔渲染引擎、精选模板库、可选 AI 旁白或源视频人声。Apache-2.0，无单次渲染费用，不绑定厂商。
...
<p align="center">
  ...
  <a href="#作品展示"><img alt="Templates" src="https://img.shields.io/badge/templates-37-3ce6ac?style=flat-square" /></a>
  ...
  <a href="#音频与口播视频"><img alt="Audio" src="https://img.shields.io/badge/audio-AI%20voice%20%C2%B7%20talking--head-e67e22?style=flat-square" /></a>
...
……还有 31 个，包括多场景产品宣传、动感排版、瑞士网格与 Vignelli 数据卡、决策树解说、Takram 有机动效、暖色颗粒杂志风，以及 Remotion 原生数据滚动模板。全部 38 个都可在 studio 模板库里查看静态视频帧示例。
...
| [Remotion](https://www.remotion.dev/) | React 组件 | source-available，4 人以上收费 | ✅ **可选增强** —— 通过 `frame-data-rollup` 做原生数据帧渲染 |
...
> **状态：** 可插拔引擎架构已就位。**Hyperframes 引擎已完整接通、能渲染出真实 MP4** —— 无头 Chromium 录制带动画的 HTML，再用 ffmpeg 编码（libx264）。**Remotion 也已作为可选逐帧增强接入**：用户可以把数据帧切到原生 `frame-data-rollup` 渲染，让数字滚动、柱子生长，其余帧仍走 Hyperframes。Motion Canvas / Revideo、Manim 仍在路线图上。
...
| **38 个模板** | 精选、许可清晰的样式：数据可视化、产品宣传、社媒短片、解说、动感排版、转场 —— 在模板库里查看静态视频帧示例。 |
| **多帧故事板** | content-graph 驱动多场景视频；逐帧改文案、数据帧 Remotion 增强、重渲染。 |
| **AI 旁白** | 可选生成旁白，导出时混进 MP4。 |
| **口播视频源** | 上传人像口播，用本地 Whisper 提取带时间字幕，导出时右下角叠加并使用源视频音轨。 |
...
```
  prompt / 链接 / 仓库 / 口播视频
        │
        ▼
  ① 来源摄取        studio 在服务端拉取 URL/仓库，或本地转写视频字幕
        │
        ▼
  ...
  ⑥ ffmpeg          每个 webm → mp4（libx264），再 concat 成一支视频；
        │            可选 AI 旁白，或源视频音轨 + 右下角画中画
        ▼
      你的.mp4
```

第 ②–④ 步正是「meta-layer」所在：agent 决定故事板，引擎决定怎么画，两者互不渗透。第 ⑤ 步是引擎相关的 —— 现在 Hyperframes 是基座渲染器，Remotion 可用于选定的数据帧增强。全程在你本机运行；网络调用只来自可选的来源抓取和可选的 AI 旁白生成。
...
在 studio 里：挑一个模板（或直接描述视频 / 粘链接 / 上传口播视频），跟 agent 对话，逐帧改文案，选择源视频人声或 AI 旁白，导出 MP4。
...
html-video 支持两种音频 / 源视频模式：

- **口播视频源** —— 在 **Talking-head source** 面板上传人像口播视频，用本地 Whisper 提取带时间字幕，再让 agent 基于字幕生成视频。导出时，口播视频会作为右下角画中画叠加，最终音轨使用它的原始声音。
- **AI 旁白** —— 在 **Settings → Audio** 填入旁白服务 API key，然后使用项目里的 **Soundtrack** 面板。

AI 旁白模式下：

- **旁白** —— 输入或草拟文案，配置的 TTS 服务会朗读并存成项目音频素材。
- **背景音乐** —— 当前版本未启用；导出时会混入旁白或源视频音轨。

旁白会通过 ffmpeg 混进导出的 MP4。如果启用了口播视频模式并选择原始音频，本次导出优先使用源视频音轨；已经保存的旁白仍保留在项目里，之后可以继续使用。

---

## 模板库

这 38 个模板不是随手凑的一堆 —— 每一个都是自包含、agent 可读的单元，由一份 `template.html-video.yaml` 清单描述，studio 启动时扫描加载。一份清单带齐了 agent 挑选和驱动这个模板所需的一切，根本不用打开 HTML：
...
│                          百炼 CosyVoice / MiniMax provider + ffmpeg 媒体后处理
...
├── adapter-remotion/      Remotion 适配器 —— 可选原生数据帧增强
├── cli/                   `html-video` 命令 + studio HTTP server + 来源抓取
└── project-studio/        浏览器 studio UI（对话、模板库、帧、口播视频、音频、导出）
templates/                 38 个精选、许可清晰的视频模板
...
| [Remotion](https://www.remotion.dev/) | 可选原生数据帧渲染器，用于数字滚动 / 柱状增长；较大商业团队需自行确认 Remotion 许可 |
