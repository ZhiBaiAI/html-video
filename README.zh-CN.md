...
```
  ...
  ⑥ ffmpeg          每个 webm → mp4（libx264），再 concat 成一支视频；
        │            可选 AI 配乐，或源视频音轨 + 右下角画中画
        ▼
      你的.mp4
```

第 ②–④ 步正是「meta-layer」所在：agent 决定故事板，引擎决定怎么画，两者互不渗透。第 ⑤ 步是引擎相关的 —— 以后换成 Remotion 或 Motion Canvas，只替换这一个环节，故事板和 agent 循环原封不动。全程在你本机运行；网络调用只来自可选的来源抓取和可选的 AI 配乐。
...
在 studio 里：挑一个模板（或直接描述视频 / 粘链接 / 上传口播视频），跟 agent 对话，逐帧改文案，选择源视频人声或 AI 配乐，导出 MP4。
...
- **AI 配乐** —— 在 **Settings → Audio** 填入 MiniMax API key，然后使用项目里的 **Soundtrack** 面板。
...
音乐和旁白会通过 ffmpeg 混进导出的 MP4（音乐压低到人声之下，可选淡入淡出）。如果启用了口播视频模式，本次导出优先使用源视频音轨；已经保存的 AI 配乐仍保留在项目里，之后可以继续使用。
...
└── project-studio/        浏览器 studio UI（对话、模板库、帧、口播视频、配乐、导出）
